...
export default function Desktop(props: MacActions) {
  ...
  useEffect(() => {
    ...
    const handleOpenSiri = () => openApp("siri");
    const handleCloseSiri = () => closeApp("siri");
    const handleDesktopOpenApp = (e: any) => {
      if (e.detail?.id) openApp(e.detail.id);
    };
    const handleDesktopCloseApp = (e: any) => {
    ...
    window.addEventListener("desktop:openApp", handleDesktopOpenApp);
    window.addEventListener("desktop:closeApp", handleDesktopCloseApp);
    ...
    return () => {
      ...
      window.removeEventListener("desktop:openApp", handleDesktopOpenApp);
      window.removeEventListener("desktop:closeApp", handleDesktopCloseApp);
    ...
    >
      {isVideoWallpaper && (
        <video
          ...
          style={{ filter: `brightness(${(brightness as number) * 0.7 + 50}%)` }}
        />
      )}
      {/* Top Menu Bar */}
      <TopBar
      ...
      <SiriVoiceAssistant
        isSiriOpen={!!state.showApps["siri"]}
